...
public class DeplacementTask extends Thread {
	...
	private void match() throws IOException {
		...
		marcheAvant = true;
		asserv.gotoPosition(320, mult*200);
		this.waitForFinish();
		asserv.gotoPosition(1520, mult*200);
		this.waitForFinish();
			
		// On monte sur la première ligne
		asserv.gotoPosition(1520, mult*500);
		servos.ouvertureTourVerres();
		this.waitForFinish();
			
		// On revient avec la pince ouverte
		asserv.gotoPosition(370, mult*500);
		this.waitForFinish();
		...
		asserv.gotoPosition(1520, mult*750);
		servos.ouvertureTourVerres();
		this.waitForFinish();
			
		// On revient avec la pince ouverte
		asserv.gotoPosition(370, mult*750);
		this.waitForFinish();
		...
		servos.fermetureTourVerres();
		marcheAvant = true;
		asserv.gotoPosition(1530, mult*750);
		this.waitForFinish();
			
		// On monte sur la troisième ligne
		asserv.gotoPosition(1530, mult*1000);
		servos.ouvertureTourVerres();
		this.waitForFinish();
			
		// On revient
		asserv.gotoPosition(370, mult*1000);
		this.waitForFinish();
