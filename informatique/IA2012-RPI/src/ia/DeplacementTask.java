...
public class DeplacementTask extends Thread /*implements Runnable*/ {
	...
	private void matchBien() {
		...
		asservissement.gotoEnchaine(878, mult*100); //On esquive le CD
		...
		waitForFinish();
		asservissement.gotoEnchaine(578, mult*540); //On râcle
		...
		waitForFinish();
		detectionActive = false;
		asservissement.go(300); //On redonne un petit coup
		marche_avant = true;
		waitForFinish();
		detectionActive = true;
		asservissement.go(-300); //On recule pour avoir la place de tourner
		marche_avant = false;
		waitForFinish();
		asservissement.gotoEnchaine(478, mult*450); //On remonte un peu
		marche_avant = true;
		...
		marche_avant = false;
		waitForFinish();
		asservissement.gotoEnchaine(500, mult*678); //On reprend la ligne
		marche_avant = true;
		asservissement.gotoEnchaine(500, mult*1178); //On descend
		...
		asservissement.go(-100); //On recule des éléments (peut-être)
		try {
			Thread.sleep(500); //On met une tempo, parce qu'on est quasi sûr d'être bloqué
		} catch(Exception e) {
			e.printStackTrace();
		}
		asservissement.go(50); //On avance dans une zone où on a supposément fait le ménage
		try {
			Thread.sleep(500); //On met une tempo, parce qu'on est quasi sûr d'être bloqué
		} catch(Exception e) {
			e.printStackTrace();
		...
		*/
		...
		while(true) {
			...
			asservissement.go(-100); //On fait des va-et-vient pour tout bien décharger
			waitForFinish();
			asservissement.go(100); //On fait des va-et-vient pour tout bien décharger
			waitForFinish();
